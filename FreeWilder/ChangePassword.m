...
@interface ChangePassword ()<UIPickerViewDelegate,UIPickerViewDataSource,UIGestureRecognizerDelegate,Slide_menu_delegate,footerdelegate,Serviceview_delegate,Profile_delegate,accountsubviewdelegate,accountsubviewdelegate,sideMenu,UITextFieldDelegate>
{
...
{
    if ([self TarminateWhiteSpace:changePass.text].length==0 || [self TarminateWhiteSpace:confirmPass.text].length==0)
    {
        changePass.placeholder=@"Please enter valid password";
         confirmPass.placeholder=@"Please enter valid password";
    }
    
    else if (![[changePass text ] isEqualToString:[confirmPass text]])
    {
        changePass.text=@"";
        confirmPass.text=@"";
        changePass.placeholder=@"Password Mismatch";
        confirmPass.placeholder=@"Password Mismatch";
        
    }
    else if (changePass.text.length<6 || confirmPass.text.length <6)
    {
        changePass.text=@"";
        confirmPass.text=@"";
        changePass.placeholder=@"Enter min 6 digit password";
        confirmPass.placeholder=@"Enter min 6 digit password";
    }
    else
    {
        [changePass resignFirstResponder];
        ...
        NSString *curr_id;
        curr_id=[prefs valueForKey:@"UserId"];
        NSString *url = [NSString stringWithFormat:@"%@/app_change_password?userid=%@&n_pass=%@",App_Domain_Url,curr_id,[confirmPass text]];
        NSLog(@"%@",url);
        [globalobj GlobalDict:url Globalstr:@"array" Withblock:^(id result, NSError *error) {
            ...
            NSMutableArray *data = [[result valueForKey:@"infoarray" ]mutableCopy];
            
            if ([[data valueForKey:@"response"]isEqualToString:@"success"])
            {
                ...
                UIAlertView *alrt = [[UIAlertView alloc]initWithTitle:[data valueForKey:@"response"] message:[data valueForKey:@"message"] delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
