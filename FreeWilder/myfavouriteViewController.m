...
@interface myfavouriteViewController ()<UITableViewDataSource,UITableViewDelegate,sideMenu,footerdelegate,UIGestureRecognizerDelegate,UITextFieldDelegate>
{
...
-(void)urlfire
{
    ...
    NSString *urlstring=[NSString stringWithFormat:@"%@app_user_favourite?userid=%@",App_Domain_Url,userid];
    NSLog(@"str=%@",urlstring);
...
{
    myfavouriteTableViewCell *cell=(myfavouriteTableViewCell *)[tableView dequeueReusableCellWithIdentifier:@"favouritecell"];
    
    
    
    cell.business_name.text=[NSString stringWithFormat:@"%@",[[dataarray objectAtIndex:indexPath.row] valueForKey:@"business_name"]] ;
    cell.fav_user_name.text=[[dataarray objectAtIndex:indexPath.row] valueForKey:@"fav_user_name"] ;
    cell.location.text=[NSString stringWithFormat:@"%@",[[dataarray  objectAtIndex:indexPath.row] valueForKey:@"location"] ];
    cell.business_desc.text=[NSString stringWithFormat:@"%@",[[dataarray objectAtIndex:indexPath.row]valueForKey:@"business_desc"]] ;
    
    cell.fav_user_img.layer.cornerRadius = cell.fav_user_img.frame.size.width/2;
    cell.fav_user_img.clipsToBounds = YES;
    //cell.fav_user_img.userInteractionEnabled=YES;
    cell.fav_user_img.layer.borderColor=[UIColor whiteColor].CGColor;
    cell.fav_user_img.layer.borderWidth=1.5;
    cell.fav_user_img.contentMode=UIViewContentModeScaleAspectFill;
    
    
    [cell.fav_user_img sd_setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",[[dataarray valueForKey:@"fav_user_img"] objectAtIndex:indexPath.row]]] placeholderImage:[UIImage imageNamed:@"ProfileImage"] options:/* DISABLES CODE */ (0) == 0?SDWebImageRefreshCached : 0];
    ...
    button = [UITableViewRowAction rowActionWithStyle:UITableViewRowActionStyleDefault title:@"Remove" handler:^(UITableViewRowAction *action, NSIndexPath *indexPath)
              {
                  ...
                  NSLog(@"favid%@",[[dataarray objectAtIndex:indexPath.row]valueForKey:@"favourite_id"] );
                  
                  
                  NSString *urlstring=[NSString stringWithFormat:@"%@app_delete_fav?fav_id=%@",App_Domain_Url,[[dataarray objectAtIndex:indexPath.row] valueForKey:@"favourite_id"] ];
                  NSLog(@"str=%@",urlstring);
                  ...
                  [globalobj GlobalDict:urlstring Globalstr:@"array" Withblock:^(id result, NSError *error)
                   {
                       ...
                       NSString *datachk=[result valueForKey:@"response"];
