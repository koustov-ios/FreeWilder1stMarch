...
@interface SettingsViewController ()<UIPickerViewDelegate,UIPickerViewDataSource,UIGestureRecognizerDelegate,footerdelegate,sideMenu,UITextFieldDelegate>
{
    //FW_JsonClass *obj;
    
    NSMutableArray *countryArry,*country_idArray;
    NSString *holdCountry;
    NSString *userid,*country_id,*countryname;
...
#pragma mark - Footer variables initialization
    ...
    //Footer ends here------>
     ...
     [globalobj GlobalDict:[NSString stringWithFormat:@"%@/app_user_settings?userid=%@",App_Domain_Url,userid] Globalstr:@"array" Withblock:^(id result, NSError *error)
      {
          
          
          if ([[result valueForKey:@"response"]isEqualToString:@"success"])
          {
              countryArry = [result valueForKey:@"infoarray"];
              
              country_id=[countryArry valueForKey:@"country_id"];
              
              countryname = [countryArry valueForKey:@"country_name"];
    ...
    [globalobj GlobalDict:curl Globalstr:@"array" Withblock:^(id result, NSError *error)
     {
         ...
         temp1= [[result valueForKey:@"infoarray" ] mutableCopy];
         
         
         
         countryname = [[temp1 objectAtIndex:0]valueForKey:@"country_name"];
         country_id = [[temp1 objectAtIndex:0]valueForKey:@"country_id"];
         ...
         for (int i=0 ; i<temp1.count; i++)
         {
             ...
             NSString *tempcounry = [[temp1 objectAtIndex:i]valueForKey:@"country_name"];
             NSString *temp_id = [[temp1 objectAtIndex:i]valueForKey:@"country_id"];
...
- (IBAction)SaveCountry:(id)sender
{
    ...
    NSString *url=[NSString stringWithFormat:@"%@app_settings_country_update?userid=%@&country_id=%@",App_Domain_Url,userid,country_id];
